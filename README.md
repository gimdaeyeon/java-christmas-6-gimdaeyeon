...
- ### 사용자 관점
  ...
  2. 주문할 메뉴와 개수를 입력한다.
  3. 주문한 메뉴와 갯수가 출력된다.
  4. 방문 날짜와 주문 금액에 따른 할인 전 총금액, 혜택 내역, 혜택 금액, 할인 후 결제 금액, 이벤트 벳지 내용들을 확인한다
- ### 개발자(내부 프로그램) 관점
  1. 프로그램이 시작되면 식당에서 판매할 메뉴들을 등록한다.
  2. 특별할인이 적용될 날짜(달력에 🌟이 있는 날짜)를 등록한다
  3. 안내메시지 출력 후 사용자에게 방문 날짜를 입력받는다(입력값 검증, 유효하지 않을 시 재입력 받기)
  4. 안내메시지 출력 후 사용자에게 주문할 메뉴와 개수를 입력받는다.(입력값 검증, 유효하지 않을 시 재입력 받기) 
  5. 입력받은 날짜와 메뉴들을 토대로 해당 사용자에게 적용 될 할인 내역들을 전체 출력한다.
...
- 날짜 입력 검증
  - [x] 입력값이 숫자인지 검증
  - [x] 입력값이 이벤트 적용 월의 시작일과 마지막일 사이의 값인지 검증
- 주문 메뉴, 개수 입력 검증
  - [x] 주문 입력 형식이 올바른 형식인지 검증
  - [x] 입력값 메뉴가 메뉴판에 있는 메뉴인지 검증
  - [x] 입력된 메뉴가 음료로만 구성되어 있는지 검증
  - [x] 입력된 각 메뉴의 갯수가 1이상 20이하의 숫자인지 검증
  - [x] 사용자가 중복된 메뉴를 입력했는지 검증
  - [x] 전체 입력된 메뉴들의 수가 20개가 넘는지 검증

### 프로그램 진행
- [ ] 할인 전 총 주문 금액 계산
- [ ] 총 주문 금액이 10000원 이하인지 확인
- [ ] 입력받은 메뉴, 개수로 주문리스트 만들기

### 날짜
- [ ] 입력받은 날짜가 평일인지 주말인지 확인
- [x] 입력받은 날짜에 특별할인이 적용되는지 확인

### 할인
- [ ] 입력받은 날짜로 크리스마스 디데이 할인금액 구하기
- [ ] 방문날짜가 평일일 때 적용되는 할인금액 계산 
- [ ] 방문날짜가 주말일 때 적용되는 할인금액 계산 
- [ ] 방문날짜가 특별 할인 적용날짜일 때 적용되는 할인금액 계산 
- [ ] 총 주문 금액이 120000원 이상인지 확인(25,000 샴페인 증정)
- [ ] 적용된 모든 할인 금액 계산
- [ ] 혜택 금액에 따라 이벤트 배지 부여
